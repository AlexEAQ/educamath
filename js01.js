...
var textoDivision1,
  textoDivision2,
  textoDivision3,
  textoMultiplicacion1,
  textoMultiplicacion2,
  textoRaiz1,
  textoDFactorial1;
var divext = false;
...
function checkLanguage() {
	navigator.globalization.getPreferredLanguage(
		function (language) {
			...
			if (idiomaS=="English") idioma=2;
			else if (idiomaS=="catalÃ ") idioma=1;
			else idioma=0;
		},
		function () {
			idiomaS="error";
	...
	//alert (idioma+"/"+idiomaS);
}
/**/

function alCargar() {
  document.getElementsByTagName("header")[0].innerHTML =
    "<p>CALCULADORA <span>EducaMath</span></p><p id='ptwitter' style='font-family:Bungee Spice;font-Size:0.3em;'><a id='atwitter' href='https://twitter.com/JosePolLezcano' target='_blank'></a></p>";

  
  textoDivision1 =
    "<p class='perror'>El dividendo es cero, por lo tanto el resultado es cero.</p>";
  textoDivision2 =
    "<p class='perror'>El divisor es cero, por lo tanto no existe solución a la división propuesta.</p>";
  textoDivision3 =
    "<p class='perror'>El dividendo y el divisor son cero, por lo tanto no existe solución a la división propuesta.</p>";
  textoMultiplicacion1 =
    "<p class='perror'>Estás multiplicando por cero, por lo tanto el resultado es cero.</p>";
  textoMultiplicacion2 =
    "<p class='perror'>El resultado obtenido es demasiado grande.</p>";
  textoRaiz1 = "<p class='perror'>La raíz cuadrada de cero es cero.</p>";
  textoDFactorial1 =
    "<p class='perror'>No tiene sentido hacer la descomposición factorial de cero.</p>";

  w = window.innerWidth;
  h = window.innerHeight;
  prop = h / w;

  // Esto de aquí abajo es para ajustar publicidad en horizonal y vertical.
  /*if (prop > 0.60) {
		if (prop > 1.89) {
			w = window.innerWidth;
		}
		else {
			w = window.innerHeight / 1.89;
		}
		//document.getElementById("publitop").style.display="none";
		...
		document.getElementById("publiabajo").style.bottom=-h/5.75+"px";
	}
    else {
		w = window.innerHeight / 1.56;
		//document.getElementById("publitop").style.display="initial";
		document.getElementById("publiizda").style.display="initial";
		document.getElementById("publidcha").style.display="initial";
		document.getElementById("publiabajo").style.display="none";
	}*/
  ...
  //document.getElementById("publitop").style.width=Math.floor((window.innerWidth-w)/5)+"px";
  //document.getElementById("publiizda").style.width=Math.floor((window.innerWidth-w)/2)+"px";
  //document.getElementById("publiizda").style.height=Math.floor((window.innerWidth-w)/6)+"px";
  //document.getElementById("publiizda").style.top=(h-Math.floor((window.innerWidth-w)/6))/2+"px";

  //document.getElementById("publidcha").style.width=Math.floor((window.innerWidth-w)/2)+"px";
  //document.getElementById("publidcha").style.height=Math.floor((window.innerWidth-w)/6)+"px";
  //document.getElementById("publidcha").style.top=(h-Math.floor((window.innerWidth-w)/6))/2+"px";
  ...
  document.getElementById("dondetodosucede").style.width = 1 * w + "px";
  document.getElementById("dondetodosucede").style.height = 0.98 * w + "px";
...
function escribir(t, y) {
  ...
  if (t == "c") {
    ...
    display.innerHTML = contDisplay.substring(0, contDisplay.length - 1);
  } else if (t == "primos") {
    desFacPri();
    bajarteclado();
  } else if (t == "raiz") {
...
function activadoBotones(contDisplay) {
  ...
  if (masRE.test(contDisplay) && !demasiadosCaracteres) {
    document.getElementById("tmas").disabled = false;
  } else {
    document.getElementById("tmas").disabled = true;
  }

  /* condiciones de activado del boton "=" */
  igualRE = /^\d+(,\d+){0,1}([\+\/x\-#]\d+(,\d+){0,1})+$/;
  if (igualRE.test(contDisplay)) {
    document.getElementById("tcal").disabled = false;
  } else {
    document.getElementById("tcal").disabled = true;
  }
  ...
  if (porRE.test(contDisplay) && !demasiadosCaracteres) {
    document.getElementById("trai").disabled = false;
  } else {
  ...
  primRE = /^\d+$/;
  if (primRE.test(contDisplay) && contDisplay.length <= 8) {
    document.getElementById("tpri").disabled = false;
  } else {
    document.getElementById("tpri").disabled = true;
  }

  /* condiciones de activado del boton "coma" */
  comaRE = /^(\d+(,\d+){0,1}[\+\/x-])*\d+$/;
  if (comaRE.test(contDisplay) && !demasiadosCaracteres) {
    document.getElementById("tcom").disabled = false;
  } else {
    document.getElementById("tcom").disabled = true;
  }
}
...
function calcular() {
  ...
  for (i = 0; i < numAr.length; i++) {
    posicionComa =
      numAr[i].search(",") != -1 ? numAr[i].search(",") + 1 : numAr[i].length;
    ...
    salida.innerHTML +=
      ...
      numerosAR[i][1] +
      "/" +
      numerosAR[i][0] / Math.pow(10, numerosAR[i][1]) +
      "</p>";
    /*while (numAr[i][0]=="0") {
  ...
  if (operador == "/" && divext == false) {
    divide(numerosAR);
  }
  if (operador == "/" && divext == true) {
    divideExt(numerosAR);
  }
...
function divide(numerosAR) {
  ...
  if (numDec2 > numerosAR[0][1]) {
    ...
    decimalesDividendo = numerosAR[0][1] - numDec2;
  }
  ...
  if (num1EsCero && !num2EsCero) {
    salida.innerHTML = textoDivision1;
  } else if (!num1EsCero && num2EsCero) {
    salida.innerHTML = textoDivision2;
  } else if (num1EsCero && num2EsCero) {
    salida.innerHTML = textoDivision3;
  } else {
    ...
    longitudsalida =
      1 + Math.max(num1.length + num2.length, num1.length + resultado.length);
      ...
      if (Math.floor(d / num2) == 0) {
        ...
        if (fila > 1) {
          ...
          if (col + 1 < num1.length) {
            numAr[fila - 1] = new Array(d + num1[col + 1], fila - 1, col + 2);
          }
        }
    ...
    for (i = 0; i < numAr.length; i++) {
      ...
      if (decimalesDividendo > 0 && i == 0) {
        ...
        celda.style.left =
          (numAr[i][0].length - decimalesDividendo - 0.5) * tamCel + "px";
        celda.style.top = "0px";
    ...
    for (i = 0; i < num2.length; i++) {
      ...
      celda[x].style.height = tamCel + "px";
      celda[x].style.fontSize = tamFuente + "px";
      if (i == 0) celda[x].style.borderLeft = "2px #F1C40F solid";
      celda[x].style.borderBottom = "2px #F1C40F solid";
    ...
    for (i = 0; i < resultado.length; i++) {
      ...
      celda[x].style.height = tamCel + "px";
      celda[x].style.fontSize = tamFuente + "px";
      celda[x].style.borderTop = "2px #F1C40F solid";
    ...
    if (decimalesDividendo > 0) {
      ...
      celda.className = "caja";
      celda.style.left =
        (inicioResultado - decimalesDividendo - 0.5) * tamCel + "px";
...
function divideExt(numerosAR) {
  ...
  if (num1EsCero && !num2EsCero) {
    ...
    inicioResultado = 1 + num1.length;
    longitudsalida =
      2 + Math.max(num1.length + num2.length, num1.length + resultado.length);
    tamCel = (0.95 * w) / longitudsalida;
    tamFuente = tamCel * multiplicadorTamFuente;
    ...
    for (i = 0; i < numAr.length; i++) {
      ...
      if (decimalesDividendo > 0 && i == 0) {
        // Això col·loca la coma en les divisions amb decimals...
        celda = document.createElement("div");
        celda.className = "caja";
        celda.style.left =
          (numAr[i][0].length - decimalesDividendo - 0.5 + 1) * tamCel + "px";
        celda.style.top = "0px";
        celda.style.width = tamCel + "px";
        celda.style.height = tamCel + "px";
        celda.style.fontSize = tamFuente + "px";
    ...
    if (decimalesDividendo > 0) {
      ...
      celda.className = "caja";
      celda.style.left =
        (inicioResultado - decimalesDividendo - 0.5 + 1) * tamCel + "px";
      celda.style.top = tamCel + "px";
      celda.style.width = tamCel + "px";
      celda.style.height = tamCel + "px";
      celda.style.fontSize = tamFuente + "px";
...
function multiplica(numerosAR) {
  ...
  if (num1 == "0" || num2 == "0") {
    ...
    tamFuente = tamCel * multiplicadorTamFuente;
    m =
      (Math.max(alturaMultiplicacion, anchuraMultiplicacion) -
        anchuraMultiplicacion) /
      2;
    numAr = new Array();
    fila = 2;
    ...
    for (i = 0; i < num1.length; i++) {
      ...
      celda.style.left =
        (anchuraMultiplicacion - num1.length + i + m) * tamCel + "px";
      celda.style.top = "0px";
      celda.style.width = tamCel + "px";
      celda.style.height = tamCel + "px";
      celda.style.fontSize = tamFuente + "px";
    ...
    celda.className = "caja";
    celda.style.left =
      (anchuraMultiplicacion - num2.length - 1 + m) * tamCel + "px";
    celda.style.top = tamCel + "px";
    celda.style.width = tamCel + "px";
    celda.style.height = tamCel + "px";
    celda.style.fontSize = tamFuente + "px";
    celda.style.color = "#F1C40F";
    celda.style.borderBottom = "2px #F1C40F solid";
    celda.innerHTML = "x";
    salida.appendChild(celda);
    for (i = 0; i < num2.length; i++) {
      celda = document.createElement("div");
      celda.className = "caja3";
      celda.style.left =
        (anchuraMultiplicacion - num2.length + i + m) * tamCel + "px";
      celda.style.top = tamCel + "px";
      celda.style.width = tamCel + "px";
      celda.style.height = tamCel + "px";
      celda.style.fontSize = tamFuente + "px";
    ...
    if (num2.length >= 1) {
      for (i = 0; i < numAr.length; i++) {
        for (j = 0; j < numAr[i][0].length; j++) {
          ...
          celda.style.left =
            (anchuraMultiplicacion - numAr[i][0].length + j - numAr[i][2] + m) *
              tamCel +
            "px";
          ...
          celda.style.height = tamCel + "px";
          celda.style.fontSize = tamFuente + "px";
          if (i == 0) celda.style.borderTop = "2px #F1C40F solid";
    ...
    if (num2.length > 1) {
      for (i = 0; i < resultadoS.length; i++) {
        ...
        celda.style.height = tamCel + "px";
        celda.style.fontSize = tamFuente + "px";
        celda.style.borderTop = "2px #F1C40F solid";
        celda.style.background = "#3b5998 ";
    ...
    if (resultadoS.length > 15)
      document.getElementById("salida").innerHTML = textoMultiplicacion2;
  }
}
...
function suma(numerosAR) {
  ...
  anchuraSuma = Math.max(
    numA[numA.length - 1].length,
    numA[numA.length - 2].length
  );
  ...
  tamFuente = tamCel * multiplicadorTamFuente;
  if (numA[numA.length - 1].length < anchuraSuma)
    numA[numA.length - 1] = " " + numA[numA.length - 1];
...
function resta(numerosAR) {
  ...
  if (
    numerosAR[0][0] / Math.pow(10, numerosAR[0][1]) >
    numerosAR[1][0] / Math.pow(10, numerosAR[1][1])
  ) {
    minuendo = numA[0];
    sustraendo = numA[1];
  } else {
  ...
  for (i = 1; i <= restaArray[0].length; i++) {
    ...
    if (!isNaN(restaArray[1][restaArray[1].length - i]))
      cifraSustraendo = parseInt(restaArray[1][restaArray[1].length - i]);
...
function desFacPri() {
  ...
  if (numIzda == 0) {
    salida.innerHTML = textoDFactorial1;
  } else {
    numIzdaArray = new Array();
    numIzdaArray[0] = numIzda;
    numIzdaArray[1] = numIzda;
    ...
    for (i = 0; i < numIzdaArray.length; i++) {
      ...
      for (j = 0; j < numeroAString.length; j++) {
        ...
        celda.style.height = tamCel + "px";
        celda.style.fontSize = tamFuente + "px";
        if (j == numeroAString.length - 1)
          celda.style.borderRight = "2px #F1C40F solid";
        //celda.style.background="#030";
